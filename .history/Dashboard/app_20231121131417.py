...
if ((dados == 'Estadual - 1º turno')
    ...
    with Q81:
        def plot_chart(estadoIndex, df):
            ...
            st.write(df_estado)
            df_masc = df_estado[['solteiro_masculino', 'casado_masculino', 'divorciado_masculino',
                                'viúvo_masculino', 'separado_judicialmente_masculino']]
            df_fem = df_estado[['solteiro_feminino', 'casado_feminino', 'divorciado_feminino',
                                'viúvo_feminino', 'separado_judicialmente_feminino']]

            fig = go.Figure()

            for i in range(len(df_masc.columns)):
                fig.add_trace(go.Bar(
                    y=[estado],
                    x=[df_masc.iloc[0, i]],
                    orientation='h',
                    name=df_masc.columns[i],
                    text=df_masc.columns[i],
                    hoverinfo="x+name",
                ...
                fig.add_trace(go.Bar(
                    ...
                    x=[-df_fem.iloc[0, i]],
                    orientation='h',
                    name=df_fem.columns[i],
                    text=df_fem.columns[i],
                    hoverinfo="x+name",
                ...
                fig.update_layout(barmode='relative',
                      bargap=0.2,
                      title=f'Pirâmide de Estado Civil - {estado}',
                      showlegend=False)
