        ...
        def plot_chart(estadoIndex, df):
            ...
            for index, row in df.iterrows():
                fig_combined.add_trace(go.Bar(
                    y=[f"{row['estado']} - Homens", f"{row['estado']} - Mulheres"],
                    x=row[['solteiro_masculino', 'casado_masculino', 'divorciado_masculino', 'viuvo_masculino', 'separado_judicialmente_masculino',
                        'solteiro_feminino', 'casado_feminino', 'divorciado_feminino', 'viuvo_feminino', 'separado_judicialmente_feminino']],
                    orientation='h',
                    name=row['estado'],
                    text=['Solteiro', 'Casado', 'Divorciado', 'Viúvo', 'Separado Judicialmente'] * 2,
                    hoverinfo='x+text',
                    marker=dict(color=['blue'] * 5 + ['pink'] * 5),
                ))
