...
if ((dados == 'Estadual - 1º turno')
    ...
    with Q81:
        def plot_chart(estadoIndex, df):
            ...
            st.write(df['separado_judicialmente_masculino'][0]*-1)
            fig.add_trace(go.Bar(
                y=['Separado judicialmente', 'Divorciado', 'Solteiro', 'Casado', 'Viúvo'],
                x=[df['separado_judicialmente_masculino'][0]*-1, df['divorciado_masculino'][0]*-1, df['solteiro_masculino'][0]*-1, df['casado_masculino'][0]*-1, df['viuvo_masculino'][0]*-1],#].iloc[0],
                orientation='h',
                name='Homens',
                hoverinfo='x+text',
