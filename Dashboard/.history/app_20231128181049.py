    ...
    with Q82:
        def plot_chart(estadoIndex, municipios, index, df):
            ...
            fig.update_layout(title='', plot_bgcolor="rgba(0,0,0,0)",
                            ...
                            xaxis=dict(tickvals=[-1*mulheres.max(), -1*mulheres.max()/2, -1*mulheres.max()/3, 0, mulheres.max()/3, 
                                        mulheres.max()/2, mulheres.max()],
                                        ticktext=[int(mulheres.max()), int(mulheres.max()/1.5), int(mulheres.max()/2), round(int(mulheres.max()/4),2), 0, mulheres.max()/3, 
                                        mulheres.max()/2, mulheres.max()]),
                                        )
