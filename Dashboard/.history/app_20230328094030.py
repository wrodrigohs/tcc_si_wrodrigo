...
if ((dados == 'Estadual - 1º turno')
    ...
    with Q24:
        def plot_chart(estadoIndex, df):
            estado = format_func_estado(estadoIndex)
            nomeEstado = [df.loc[(df['estado'] == estado, 'estado')]]
            ...
            fig_sexoMunicipio.add_trace(go.Bar(y=estado, x=homens,
                                               name='',
                                               hovertemplate='Homens: %{x:.2f}%',
                                               marker_color='#355070',
                                               orientation='h'))

            fig_sexoMunicipio.add_trace(go.Bar(y=estado, x=mulheres,
