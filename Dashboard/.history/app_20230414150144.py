...
if ((dados == 'Estadual - 1º turno')
    ...
    if (dados == 'Municipal - 1º turno'):
        ...
        with Q45:
            ...
            st.markdown("<h1 style='text-align: center; color: #000000;'>Big headline</h1>", unsafe_allow_html=True)

            # estado1 = format_func_estado(estado)
            # cidade = municipios[indexMunicipio]
            # valor = df.loc[(df['estado'] == estado1)
            #         & (df['municipio'] == cidade), 'comparecimento_percentual(%)'].values[0]
            # st.markdown(f""":grey[Comparecimento percentual de eleitores em {cidade}], 
