    ...
    if (dados == 'Municipal - 1º turno'):
        ...
        with st.empty():
            def plot_chart(estadoIndex, municipios, index, df):
                ...
                st.markdown(f"<h1 style='text-align: center; color: grey;'>Comparecimento percentual em {cidade}</h1>", unsafe_allow_html=True)
                st.markdown(f"<h1 style='text-align: center; color: #355070; font-size:50px'>{valor}</h1>", unsafe_allow_html=True)
                # fig = go.Figure(
                #     go.Indicator(
                #         value=valor,
