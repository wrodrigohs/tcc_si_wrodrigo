...
if ((dados == 'Estadual - 1º turno')
    ...
    with Q82:
        def plot_chart(estadoIndex, municipios, index, df):
            ...
            top_10_estado_civil = df[['estado', 'municipio', estado_civil + '_masculino',
                                      estado_civil + '_feminino']].sort_values(
                by=estado_civil + '_masculino', ascending=False)[:10]

            homens = [df.loc[(df['estado'] == estado)
                    & (df['municipio'] == cidade), 
                    estado_civil +'_masculino']] #df[estado_civil + '_masculino']
            mulheres = [df.loc[(df['estado'] == estado)
                    & (df['municipio'] == cidade), 
                    estado_civil +'_feminino']]#df[estado_civil + '_feminino']
